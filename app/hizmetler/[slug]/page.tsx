...
export default function HizmetDetayPage({ params }: { params: { slug: string } }) {
  ...
  const crumbs = [
    ...
    { name: service.titleTr, url: `${SITE_URL}/hizmetler/${service.slug}` },
  ];
  ...
  return (
    <>
      ...
      />
      <PageHero
        eyebrow={service.subtitleTr}
        title={service.titleTr}
        subtitle={service.descTr}
        breadcrumbs={[
          { label: "Ana Sayfa", href: "/" },
          { label: "Hizmetlerimiz", href: "/hizmetler" },
          { label: service.titleTr },
        ]}
        image={service.image}
      />
      ...
      <section className="py-16 md:py-24 px-6 bg-surface">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div>
            ...
            <a
              href={waLink(WA_MESSAGES.serviceTr(service.titleTr))}
              target="_blank"
            ...
            <p className="text-foreground-subtle text-xs mt-3">
              WhatsApp&apos;ta açılır, mesajınız önceden hazırlanmıştır.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 px-6 bg-surface-muted border-t border-brand-gold/10">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-brand-gold tracking-[0.3em] text-xs uppercase font-bold mb-3">Ritüel</p>
            <h2 className="font-serif text-3xl md:text-4xl text-foreground">Adım Adım Süreç</h2>
          </div>
      ...
      <section className="py-20 md:py-28 px-6 bg-surface border-t border-brand-gold/10">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-brand-gold tracking-[0.3em] text-xs uppercase font-bold mb-3">Diğer Hizmetler</p>
            <h2 className="font-serif text-3xl md:text-4xl text-foreground">Patron&apos;da bunlar da var</h2>
          </div>
      ...
      <section className="py-20 md:py-24 px-6 bg-surface-elevated border-t border-brand-gold/10">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="font-serif text-3xl md:text-4xl text-foreground mb-4">
            Hızlı bir mesajla rezerve edin
          </h2>
          <p className="text-foreground-muted text-sm md:text-base leading-relaxed mb-8">
            Müsait olduğunuz birkaç saat aralığını yazın; size en yakın slot&apos;u önerelim.
          </p>
          <a
            href={waLink(WA_MESSAGES.serviceTr(service.titleTr))}
            target="_blank"
