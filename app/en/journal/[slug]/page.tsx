...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  return buildMeta({
    ...
    path: `/en/journal/${article.slug}`,
    locale: "en",
    image: `${SITE_URL}${article.image}`,
  });
}

export default function ArticleEnPage({ params }: { params: { slug: string } }) {
  const article = getArticleBySlug(params.slug);
  if (!article) notFound();

  const locale = article.en;
  const url = `${SITE_URL}/en/journal/${article.slug}`;
  ...
  const crumbs = [
    ...
    { name: "Journal", url: `${SITE_URL}/en/journal` },
    ...
    <article className="min-h-screen bg-surface-muted pt-40 pb-24 relative">
      ...
      <div className="max-w-4xl mx-auto relative z-10 px-6 md:px-12">
        ...
        <div className="flex gap-3 font-sans text-[10px] tracking-[0.2em] uppercase text-brand-gold mb-8 items-center">
          ...
          <span className="text-foreground-subtle">{article.readTimeEn}</span>
          <span className="text-foreground-subtle/60" aria-hidden>·</span>
        ...
        {locale.faqs.length > 0 && (
          <section className="mt-16 pt-12 border-t border-hairline/15">
            ...
            <div className="space-y-6">
              {locale.faqs.map((f, i) => (
                <details key={i} className="group border-b border-hairline/15 pb-5 cursor-pointer">
                  <summary className="font-serif text-lg md:text-xl text-foreground flex justify-between items-center list-none">
                    <span>{f.q}</span>
                    <span className="text-brand-gold group-open:rotate-45 transition-transform" aria-hidden>+</span>
                  </summary>
        ...
        {related.length > 0 && (
          <section className="mt-16 pt-12 border-t border-hairline/15">
            <h2 className="font-serif text-xl md:text-2xl text-foreground mb-6">Related services</h2>
        ...
        {otherArticles.length > 0 && (
          <section className="mt-20 pt-12 border-t border-hairline/15">
            <h2 className="font-serif text-xl md:text-2xl text-foreground mb-8">More reading</h2>
