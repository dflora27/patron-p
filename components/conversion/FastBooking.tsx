...
export default function FastBooking() {
  ...
  return (
    <section id="booking" className="py-24 px-6 bg-surface-muted relative flex justify-center border-t border-hairline/10">
      <div className="absolute inset-0 bg-surface/40 mix-blend-multiply border-b border-brand-gold/10"></div>

      <div className="max-w-6xl w-full flex flex-col md:flex-row bg-surface border border-brand-gold/20 rounded-lg relative overflow-hidden shadow-2xl z-10">
        ...
        <div className="md:w-1/2 p-8 flex flex-col justify-center border-b md:border-b-0 md:border-r border-brand-gold/10 relative">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative z-10 px-0 md:px-8">
            ...
            <h2 className="font-serif text-4xl md:text-5xl text-foreground mb-6 leading-tight">Zamanını <span className="text-brand-gold italic">İyi Değerlendir</span></h2>
            <p className="font-sans text-foreground-subtle mb-10 font-light leading-relaxed">
            ...
            <div className="flex gap-4">
              <a href="tel:+905535737992" className="flex-1 bg-white/5 border border-hairline/20 text-foreground text-center py-4 text-xs font-bold tracking-[0.2em] hover:bg-white hover:text-black transition-colors rounded">HEMEN ARA</a>
        ...
        <form onSubmit={handleSubmit} className="md:w-1/2 flex flex-col gap-6 font-sans p-8 md:p-16 bg-[#0a0b0a] z-10">
          <input
            ...
            className="w-full bg-transparent border-b border-brand-gold/30 p-4 text-foreground focus:outline-none focus:border-brand-gold transition-colors"
            onChange={(e) => setFormData({ ...formData, name: e.target.value })}
          />

          <select
            required
            className="w-full bg-[#0a0b0a] border-b border-brand-gold/30 p-4 text-foreground focus:outline-none focus:border-brand-gold transition-colors appearance-none cursor-pointer"
          ...
          <div className="pt-6">
            ...
            <p className="text-foreground-subtle text-[10px] text-center mt-4 tracking-widest uppercase">Talebiniz doğrudan bize iletilecektir.</p>
          </div>
        </form>
      </div>
    </section>
