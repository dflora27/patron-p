...
export default function InstagramFeed() {
  ...
  return (
    <section className="py-24 md:py-32 bg-surface px-6 relative border-t border-brand-gold/5 max-w-[100vw] overflow-hidden">
      <div className="ambient-glow-green -left-40 top-40 opacity-20"></div>

      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-14 md:mb-16 gap-6">
          <div>
            <motion.h4
            ...
            >
              {isEn ? "Follow Our Social Media" : "Sosyal Medyada Biz"}
            </motion.h2>
          </div>
          <motion.a
            ...
            aria-label="Patron Instagram"
            className="flex items-center justify-center gap-3 bg-surface-elevated border border-brand-gold/30 hover:bg-brand-gold hover:text-white text-brand-gold px-6 md:px-8 py-3 md:py-4 rounded-full transition-all duration-300 group"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4 lg:gap-6">
          {DUMMY_POSTS.map((post, i) => (
            ...
            >
              <Image
                src={post.img}
                alt="Patron Instagram paylaşımı"
                fill
                sizes="(max-width: 640px) 50vw, 33vw"
                className="object-cover grayscale-[0.2] contrast-125 group-hover:grayscale-0 group-hover:scale-110 transition-all duration-[1s] ease-out"
              />
              <div className="absolute inset-x-4 top-4 flex justify-end z-20">
                <svg className="w-6 h-6 text-white drop-shadow-md opacity-80" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M2.93 5.4c0-1.36 1.11-2.47 2.47-2.47h13.2c1.36 0 2.47 1.11 2.47 2.47v13.2c0 1.36-1.11 2.47-2.47 2.47H5.4c-1.36 0-2.47-1.11-2.47-2.47V5.4zM20.2 5.4c0-.49-.4-.89-.89-.89H4.69c-.49 0-.89.4-.89.89v13.2c0 .49.4.89.89.89h14.62c.49 0 .89-.4.89-.89V5.4zm-2.9 6.6c0 2.92-2.37 5.29-5.29 5.29s-5.29-2.37-5.29-5.29 2.37-5.29 5.29-5.29 5.29 2.37 5.29 5.29zm-1.58 0c0-2.05-1.66-3.71-3.71-3.71s-3.71 1.66-3.71 3.71 1.66 3.71 3.71 3.71 3.71-1.66 3.71-3.71z" />
                </svg>
              </div>
